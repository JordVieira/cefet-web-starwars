...
const addMovieElement = ({ episode_id, title, content }) => {
  ...
  newMovieElement.innerHTML = `Episode ${decimalToRoman(
    episode_id
  )} - ${title}`;

  newMovieElement.addEventListener("click", () => {
    let introductionElement = document.querySelector("pre.introducao");
    introductionElement.innerHTML = `Episode ${decimalToRoman(
      episode_id
    )}\n${title}\n\n${content}`;
...
friendlyFetch(`${API_ENDPOINT}/films`).then((response) => {
  let data = response.results.map(({ episode_id, title, opening_crawl }) => {
    return { episode_id, title, content: opening_crawl };
  });
